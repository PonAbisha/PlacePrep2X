...
import cnAdvanced from "../data/coreSubjects/CN/cnAdvanced";

export default function CNAdvanced() {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      <header>
        ...
        <h1 className="text-4xl font-extrabold text-white tracking-tight">CN Advanced</h1>
      </header>

      <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-8 shadow-lg">
        <p className="text-slate-400 text-sm">Total Questions: {cnAdvanced.length}</p>
      </div>
      {cnAdvanced.map((q) => (
  <div
    key={q.id}
