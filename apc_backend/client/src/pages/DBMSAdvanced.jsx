...
import advanced from "../data/coreSubjects/dbms/advanced";

export default function DBMSAdvanced() {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      <header>
        ...
        <h1 className="text-4xl font-extrabold text-white tracking-tight">DBMS Advanced Topics</h1>
      </header>

      <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-8 shadow-lg">
        <p className="text-slate-400 text-sm">Total Questions: {advanced.length}</p>
      </div>
      {advanced.map((q) => (
  <div
    key={q.id}
