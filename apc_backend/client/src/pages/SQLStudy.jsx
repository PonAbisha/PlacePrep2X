...
export default function SQLStudy() {
  const navigate = useNavigate();

  const studyOptions = [
    { name: "Questions", icon: BookOpen, path: "/practice/core/study/SQL/questions" },
    {name: "Cheat Sheet", icon: BookOpen, path: "/practice/core/study/SQL/cheat-sheet"},
  ];
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      <header>
        ...
        <h1 className="text-4xl font-extrabold text-white tracking-tight">Structured Query Language</h1>
        <p className="text-slate-400 mt-3 text-lg max-w-2xl leading-relaxed">
          Master SQL concepts including queries, joins, subqueries, and database design principles.</p>
      </header>
        ...
        {studyOptions.map((option) => {
          return (
            ...
            >
              ...
              <p className="text-slate-400 text-sm">Explore {option.name.toLowerCase()} for SQL.</p>
            </button>
          );
        })}
      </div>
