...
import cnConceptual from "../data/coreSubjects/CN/cnConceptual";

export default function CNConceptual() {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      <header>
        ...
        <h1 className="text-4xl font-extrabold text-white tracking-tight">CN Conceptual</h1>
      </header>

      <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-8 shadow-lg">
        <p className="text-slate-400 text-sm">Total Questions: {cnConceptual.length}</p>
      </div>
      {cnConceptual.map((q) => (
  <div
    key={q.id}
