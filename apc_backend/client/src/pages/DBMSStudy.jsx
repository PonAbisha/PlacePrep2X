...
export default function DBMSStudy() {
  const navigate = useNavigate();

  const studyOptions = [
    { name: "Questions", icon: BookOpen, path: "/practice/core/study/DBMS/questions" },
    { name: "Cheat Sheet", icon: FileText, path: "/practice/core/study/DBMS/cheat-sheet" },
    { name: "Interview Questions", icon: HelpCircle, path: "/practice/core/study/DBMS/interview-questions" },
  ];
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      <header>
        ...
        <h1 className="text-4xl font-extrabold text-white tracking-tight">DBMS</h1>
        <p className="text-slate-400 mt-3 text-lg max-w-2xl leading-relaxed">
          Dive deep into Database Management Systems with study materials, cheat sheets, and interview questions.
        </p>
      </header>
        ...
        {studyOptions.map((option) => {
          return (
            ...
            >
              ...
              <p className="text-slate-400 text-sm">Explore {option.name.toLowerCase()} for DBMS.</p>
            </button>
          );
        })}
      </div>
