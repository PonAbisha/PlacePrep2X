...
import cnScenarioBased from "../data/coreSubjects/CN/cnScenarioBased";

export default function CNScenarioBased() {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      <header>
        <button
          onClick={() => navigate("/practice/core/study/CN/questions")}
        ...
        <h1 className="text-4xl font-extrabold text-white tracking-tight">CN Scenario-Based</h1>
      </header>

      <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-8 shadow-lg">
        <p className="text-slate-400 text-sm">Total Questions: {cnScenarioBased.length}</p>
      </div>
      {cnScenarioBased.map((q) => (
  <div
    key={q.id}
